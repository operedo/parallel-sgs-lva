...
! set global variable to be threadprivate for OpenMP usage
  type(tree_search_record), save, target :: sr   ! A GLOBAL VARIABLE for search
!$omp threadprivate(sr)

contains
  function kdtree2_create_v2(nx,ny,nz,NODES,sort,rearrange) result (mr)
  use global
    !
    ...
    integer nx,ny,nz,NODES,i,j

    allocate (mr)

!$omp single    
    if(kdkind.eq.kind(0.0))then
        do i=1,NODES
        do j=1,d_tree
            coord_ISOMAP_trans_tmp(j,i)=real(coord_ISOMAP_trans(j,i))
        end do
        end do 
        write(*,*)'Copied dp to sp coord_ISOMAP_trans array'
    end if
!$omp end single
!$omp barrier
    mr%the_data => coord_ISOMAP_trans_tmp(1:d_tree,:)
    mr%IS_USABLE => is_usable
    ! pointer assignment

    mr%dimen = d_tree
    
    mr%n = size(mr%the_data,2)
...
!$omp single
       do i=1,mr%n
          coord_ISOMAP_trans_rearranged(:,i) = mr%the_data(:,mr%ind(i))
       enddo
!$omp end single
!$omp barrier
       mr%rearranged_data => coord_ISOMAP_trans_rearranged(1:d_tree,:)  
    else
       nullify(mr%rearranged_data)
       nullify(mr%REARRANGED_IS_USABLE)    !***NEW***
    endif

  end function kdtree2_create_v2
  ...
  recursive subroutine search_opt01(node)
    ...
    if ((associated(node%left) .and. associated(node%right)) .eqv. .false.) then
       ...
       if (sr%nn .eq. 0) then
          ...
          !call process_terminal_node(node)
          !call process_terminal_node_opt01(node)
          call process_terminal_node_opt03(node)
          !call process_terminal_node_opt04(node)
       endif
    else
       ! we are not on a terminal node
       qv => sr%qv(1:)
...
!          dis=extra**2
  ...
  subroutine process_terminal_node_opt03(node)
    !
    ! Look for actual near neighbors in 'node', and update
    ! the search results on the sr data structure.
    !
    ...
    if (rearrange) then
       mainlooprea: do i = nodel, nodeu
          ...
          do k = 1,4,dimen-3
             sd = sd + (data(k,i) - qv(k))**2
             !if (sd>ballsize) cycle mainlooprea
             sd = sd + (data(k+1,i) - qv(k+1))**2
             sd = sd + (data(k+2,i) - qv(k+2))**2
             sd = sd + (data(k+3,i) - qv(k+3))**2
             if (sd>ballsize) cycle mainlooprea
          end do
          indexofi = ind(i)  ! only read it if we have not broken out
  ...
  end subroutine process_terminal_node_opt03
  
  subroutine process_terminal_node_opt04(node)
    !
    ! Look for actual near neighbors in 'node', and update
    ! the search results on the sr data structure.
    !
  ...
  end subroutine process_terminal_node_opt04
